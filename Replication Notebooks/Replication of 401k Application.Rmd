...
```{r}
Predict_torchNN <- function(model, X_new, lambda = 0.01) {
   ...
   pred_forwardpass <- as.numeric(model(X_t))
   
  return(list(predictions = pred_dual, alt_prediction = pred_dual))
}
```
...
love_plot_aipwatt = create_love_plot("AIPW-ATT", 5)
love_plot_aipwatu = create_love_plot("AIPW-ATU", 6)
love_plot_plr
love_plot_plriv
love_plot_aipw
love_plot_waipw
...
figure2 = grid.arrange(
  ...
  love_plot_aipwatt, love_plot_aipwatu,
  nrow = 2
)
```
